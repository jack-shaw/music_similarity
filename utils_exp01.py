...
# Experiment 1: Sim(Mixture(No drum), Mixture) vs Sim(Mixture(No vocal), Mixture)
...
def create_mixture_without_component_segment(track, component_name, start_time, end_time, sample_rate=44100):
    """
    Create a segment of the mixture with one component removed, e.g.
    mixture(no drums) = mixture - drums
    mixture(no vocals) = mixture - vocals

    Args:
        track: Track dictionary
        component_name: 'vocals' or 'drums' (or any other component in the track)
        start_time: Start time in seconds
        end_time: End time in seconds
        sample_rate: Sample rate (default: 44100)

    Returns:
        numpy array: mixture-without-component segment (mono or stereo), or None on failure
    """
    mixture_seg = extract_audio_segment_from_track(track, 'mixture', start_time, end_time, sample_rate)
    comp_seg = extract_audio_segment_from_track(track, component_name, start_time, end_time, sample_rate)

    if mixture_seg is None or comp_seg is None:
        return None

    mixture_seg = np.asarray(mixture_seg)
    comp_seg = np.asarray(comp_seg)

    # Try to subtract in-place if shapes match
    if mixture_seg.shape == comp_seg.shape:
        return mixture_seg - comp_seg

    # If shapes differ, fall back to mono subtraction
    mixture_mono = convert_audio_to_mono(mixture_seg)
    comp_mono = convert_audio_to_mono(comp_seg)

    if mixture_mono is None or comp_mono is None:
        return None

    min_len = min(len(mixture_mono), len(comp_mono))
    if min_len == 0:
        return None

    mixture_mono = mixture_mono[:min_len]
    comp_mono = comp_mono[:min_len]

    return mixture_mono - comp_mono


def compute_nocomp_vs_mixture_similarity(track, start_time, end_time,
                                         model_type, feature_extractor=None,
                                         model=None, sample_rate=44100, metric='cosine',
                                         normalize_rms=False, target_rms=0.1):
    """
    Compute similarity between:
      - Mixture without drums  vs full mixture
      - Mixture without vocals vs full mixture
    ...
    Args:
        ...
        normalize_rms: If True, normalize the "no-component" mixtures before feature extraction
        target_rms: Target RMS for normalization

    Returns:
        dict: {
            'no_drum_sim': Sim(Mix(no drum), Mix),
            'no_vocal_sim': Sim(Mix(no vocal), Mix),
            'start_time': start_time,
            'end_time': end_time
        }
        or None on failure
    """
    # Full mixture segment for reference
    mixture_seg = extract_audio_segment_from_track(track, 'mixture', start_time, end_time, sample_rate)
    if mixture_seg is None:
        return None

    # Mixture with components removed
    mix_no_drum_seg = create_mixture_without_component_segment(track, 'drums', start_time, end_time, sample_rate)
    mix_no_vocal_seg = create_mixture_without_component_segment(track, 'vocals', start_time, end_time, sample_rate)

    if mix_no_drum_seg is None or mix_no_vocal_seg is None:
        return None

    # Extract features
    mix_no_drum_feat = extract_segment_features(
        mix_no_drum_seg, model_type, feature_extractor, model,
        sample_rate, normalize_rms=normalize_rms, target_rms=target_rms
    )
    mix_no_vocal_feat = extract_segment_features(
        mix_no_vocal_seg, model_type, feature_extractor, model,
        sample_rate, normalize_rms=normalize_rms, target_rms=target_rms
    )
    mix_feat = extract_segment_features(
        mixture_seg, model_type, feature_extractor, model,
        sample_rate, normalize_rms=False
    )

    if mix_no_drum_feat is None or mix_no_vocal_feat is None or mix_feat is None:
        return None

    # Ensure 2D
    if len(mix_no_drum_feat.shape) == 1:
        mix_no_drum_feat = mix_no_drum_feat.reshape(1, -1)
    if len(mix_no_vocal_feat.shape) == 1:
        mix_no_vocal_feat = mix_no_vocal_feat.reshape(1, -1)
    if len(mix_feat.shape) == 1:
        mix_feat = mix_feat.reshape(1, -1)

    # Compute similarities
    if metric == 'cosine':
        no_drum_sim = cosine_similarity(mix_no_drum_feat, mix_feat)[0, 0]
        no_vocal_sim = cosine_similarity(mix_no_vocal_feat, mix_feat)[0, 0]
    elif metric == 'euclidean':
        no_drum_dist = euclidean_distances(mix_no_drum_feat, mix_feat)[0, 0]
        no_vocal_dist = euclidean_distances(mix_no_vocal_feat, mix_feat)[0, 0]
        no_drum_sim = 1 / (1 + no_drum_dist)
        no_vocal_sim = 1 / (1 + no_vocal_dist)
    else:
        raise ValueError(f"Unknown metric: {metric}")

    return {
        'no_drum_sim': no_drum_sim,
        'no_vocal_sim': no_vocal_sim,
        'start_time': start_time,
        'end_time': end_time
    }


def run_experiment1(tracks, model_type, feature_extractor=None, model=None,
                    segment_duration=10.0, sample_rate=44100, max_segments_per_track=3,
                    normalize_rms=False, target_rms=0.1):
    """
    Run Experiment 1:
        Sim(Mixture(no drum), Mixture) vs Sim(Mixture(no vocal), Mixture)
    ...
    Args:
        ...
        sample_rate: Sample rate (default: 44100)
        max_segments_per_track: Maximum number of segments per track (default: 3)
        normalize_rms: If True, normalize "no-component" mixtures' RMS (default: False)
        target_rms: Target RMS level if normalize_rms is True

    Returns:
        dict: Results dictionary with similarities and statistics:
            {
                'track_names': [...],
                'no_drum_similarities': np.array([...]),
                'no_vocal_similarities': np.array([...]),
                'segments_info': [...],
                'stats': {
                    'mean_no_drum_sim': ...,
                    'mean_no_vocal_sim': ...,
    ...
    results = {
        'track_names': [],
        'no_drum_similarities': [],
        'no_vocal_similarities': [],
        'segments_info': []
    }

    for track in tracks:
        track_name = track['name']
        print(f"\nProcessing track (Exp1): {track_name}")

        # Segments where vocals, drums, and mixture all have content
        ...
        for start_time, end_time in segments_to_use:
            sim_result = compute_nocomp_vs_mixture_similarity(
            ...
            if sim_result is not None:
                ...
                results['no_drum_similarities'].append(sim_result['no_drum_sim'])
                results['no_vocal_similarities'].append(sim_result['no_vocal_sim'])
                results['segments_info'].append({
                    'start': start_time,
                    'end': end_time
                })
                print(f"    Segment [{start_time:.1f}s-{end_time:.1f}s]: "
                      f"Mix(no drum) sim={sim_result['no_drum_sim']:.4f}, "
                      f"Mix(no vocal) sim={sim_result['no_vocal_sim']:.4f}")

    # Convert to numpy arrays
    results['no_drum_similarities'] = np.array(results['no_drum_similarities'])
    results['no_vocal_similarities'] = np.array(results['no_vocal_similarities'])

    # Compute statistics
    if len(results['no_drum_similarities']) > 0:
        results['stats'] = {
            'mean_no_drum_sim': np.mean(results['no_drum_similarities']),
            'mean_no_vocal_sim': np.mean(results['no_vocal_similarities']),
            'std_no_drum_sim': np.std(results['no_drum_similarities']),
            'std_no_vocal_sim': np.std(results['no_vocal_similarities']),
            'median_no_drum_sim': np.median(results['no_drum_similarities']),
            'median_no_vocal_sim': np.median(results['no_vocal_similarities']),
            'num_segments': len(results['no_drum_similarities'])
        }
    else:
        results['stats'] = {
            'mean_no_drum_sim': np.nan,
            'mean_no_vocal_sim': np.nan,
            'std_no_drum_sim': np.nan,
            'std_no_vocal_sim': np.nan,
            'median_no_drum_sim': np.nan,
            'median_no_vocal_sim': np.nan,
            'num_segments': 0
        }
...
def plot_experiment1_results(openl3_results, audiomae_results, figsize=(14, 10)):
    """
    Visualize Experiment 1 results comparing OpenL3 and AudioMAE.

    Args:
        openl3_results: Results dictionary from run_experiment1 for OpenL3
        audiomae_results: Results dictionary from run_experiment1 for AudioMAE
    ...
    axes[0, 0].scatter(
        openl3_results['no_drum_similarities'],
        openl3_results['no_vocal_similarities'],
        alpha=0.6, s=50, color=openl3_color
    )
    axes[0, 0].plot([0, 1], [0, 1], 'r--', alpha=0.5, label='y=x')
    axes[0, 0].set_xlabel('Sim(Mix(no drum), Mix)')
    axes[0, 0].set_ylabel('Sim(Mix(no vocal), Mix)')
    axes[0, 0].set_title('OpenL3: Component-Removed Mixture vs Full Mixture')
    axes[0, 0].legend()
    ...
    axes[0, 1].scatter(
        audiomae_results['no_drum_similarities'],
        audiomae_results['no_vocal_similarities'],
        alpha=0.6, s=50, color=audiomae_color
    )
    axes[0, 1].plot([0, 1], [0, 1], 'r--', alpha=0.5, label='y=x')
    axes[0, 1].set_xlabel('Sim(Mix(no drum), Mix)')
    axes[0, 1].set_ylabel('Sim(Mix(no vocal), Mix)')
    axes[0, 1].set_title('AudioMAE: Component-Removed Mixture vs Full Mixture')
    axes[0, 1].legend()
    ...
    axes[1, 0].set_title('Similarity Distribution: Component-Removed Mixtures')
    axes[1, 0].grid(axis='y', alpha=0.3)

    # Plot 4: Mean comparison bar chart with error bars and value labels
    components = ['Mix(no drum)', 'Mix(no vocal)']
    ol3_stats = openl3_results['stats']
    am_stats = audiomae_results['stats']

    openl3_means = [ol3_stats['mean_no_drum_sim'], ol3_stats['mean_no_vocal_sim']]
    audiomae_means = [am_stats['mean_no_drum_sim'], am_stats['mean_no_vocal_sim']]
    openl3_stds = [ol3_stats['std_no_drum_sim'], ol3_stats['std_no_vocal_sim']]
    audiomae_stds = [am_stats['std_no_drum_sim'], am_stats['std_no_vocal_sim']]

    x = np.arange(len(components))
    width = 0.35

    bars1 = axes[1, 1].bar(
        x - width/2, openl3_means, width,
        yerr=openl3_stds, label='OpenL3',
        alpha=0.8, color=openl3_color, capsize=5,
        error_kw={'elinewidth': 2, 'capthick': 2}
    )
    bars2 = axes[1, 1].bar(
        x + width/2, audiomae_means, width,
    ...
    def add_value_labels(bars, values, stds):
        for bar, val, std in zip(bars, values, stds):
            ...
            axes[1, 1].text(
                bar.get_x() + bar.get_width()/2., height + std + 0.01,
                f'{val:.3f}',
    ...
    axes[1, 1].set_title('Mean Similarity: Component-Removed vs Full Mixture', fontsize=12, fontweight='bold')
    ...
    axes[1, 1].set_ylim([0, max(
        max(openl3_means) + max(openl3_stds),
        max(audiomae_means) + max(audiomae_stds)
    ) * 1.2])

    plt.tight_layout()

    summary_text = generate_model_comparison_summary_experiment1(openl3_results, audiomae_results)
    print("\n" + "=" * 80)
    print("EXPERIMENT 1 MODEL COMPARISON SUMMARY")
    print("=" * 80)
    print(summary_text)
    print("=" * 80 + "\n")
...
def generate_model_comparison_summary_experiment1(openl3_results, audiomae_results):
    """
    Generate a text summary comparing OpenL3 and AudioMAE for Experiment 1:
    Sim(Mix(no drum), Mix) vs Sim(Mix(no vocal), Mix)
    """
    ol3 = openl3_results['stats']
    am = audiomae_results['stats']

    ol3_no_drum_mean = ol3['mean_no_drum_sim']
    ol3_no_drum_std = ol3['std_no_drum_sim']
    ol3_no_vocal_mean = ol3['mean_no_vocal_sim']
    ol3_no_vocal_std = ol3['std_no_vocal_sim']
    ol3_num_segments = ol3['num_segments']

    am_no_drum_mean = am['mean_no_drum_sim']
    am_no_drum_std = am['std_no_drum_sim']
    am_no_vocal_mean = am['mean_no_vocal_sim']
    am_no_vocal_std = am['std_no_vocal_sim']
    am_num_segments = am['num_segments']

    # "Sensitivity" here: which removal changes similarity more
    ol3_more_affected_by_drums = ol3_no_drum_mean < ol3_no_vocal_mean
    am_more_affected_by_drums = am_no_drum_mean < am_no_vocal_mean

    no_drum_diff = ol3_no_drum_mean - am_no_drum_mean
    no_vocal_diff = ol3_no_vocal_mean - am_no_vocal_mean

    summary = []
    summary.append(f"Total Segments Analyzed: OpenL3={ol3_num_segments}, AudioMAE={am_num_segments}")
    summary.append("")
    summary.append("Sim(Mix(no drum), Mix):")
    summary.append(f"  OpenL3:  {ol3_no_drum_mean:.4f} ± {ol3_no_drum_std:.4f}")
    summary.append(f"  AudioMAE: {am_no_drum_mean:.4f} ± {am_no_drum_std:.4f}")
    summary.append(f"  Difference (OpenL3 - AudioMAE): {no_drum_diff:+.4f}")
    summary.append("")
    summary.append("Sim(Mix(no vocal), Mix):")
    summary.append(f"  OpenL3:  {ol3_no_vocal_mean:.4f} ± {ol3_no_vocal_std:.4f}")
    summary.append(f"  AudioMAE: {am_no_vocal_mean:.4f} ± {am_no_vocal_std:.4f}")
    summary.append(f"  Difference (OpenL3 - AudioMAE): {no_vocal_diff:+.4f}")
    summary.append("")
    summary.append("MODEL SENSITIVITY (which removal changes similarity more):")
    if ol3_more_affected_by_drums:
        summary.append(
            f"  OpenL3:  Removing DRUMS affects similarity more "
            f"(Mix(no drum) mean={ol3_no_drum_mean:.4f} < Mix(no vocal) mean={ol3_no_vocal_mean:.4f})"
        )
    else:
        summary.append(
            f"  OpenL3:  Removing VOCALS affects similarity more "
            f"(Mix(no vocal) mean={ol3_no_vocal_mean:.4f} <= Mix(no drum) mean={ol3_no_drum_mean:.4f})"
        )

    if am_more_affected_by_drums:
        summary.append(
            f"  AudioMAE: Removing DRUMS affects similarity more "
            f"(Mix(no drum) mean={am_no_drum_mean:.4f} < Mix(no vocal) mean={am_no_vocal_mean:.4f})"
        )
    else:
        summary.append(
            f"  AudioMAE: Removing VOCALS affects similarity more "
            f"(Mix(no vocal) mean={am_no_vocal_mean:.4f} <= Mix(no drum) mean={am_no_drum_mean:.4f})"
        )

    summary.append("")
    summary.append("COMPARISON:")
    if abs(no_drum_diff) > abs(no_vocal_diff):
        if no_drum_diff > 0:
            summary.append(f"  OpenL3 has HIGHER similarity for Mix(no drum) by {abs(no_drum_diff):.4f}")
        else:
            summary.append(f"  AudioMAE has HIGHER similarity for Mix(no drum) by {abs(no_drum_diff):.4f}")
    else:
        if no_vocal_diff > 0:
            summary.append(f"  OpenL3 has HIGHER similarity for Mix(no vocal) by {abs(no_vocal_diff):.4f}")
        else:
            summary.append(f"  AudioMAE has HIGHER similarity for Mix(no vocal) by {abs(no_vocal_diff):.4f}")
